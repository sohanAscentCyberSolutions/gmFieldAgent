...
import com.example.md3.databinding.AllCasesBreakdownLayoutBinding
import com.example.md3.databinding.FragmentNewCaseBreakdownBinding
import com.example.md3.databinding.NewCaseItemBreakdownLayoutBinding
import com.example.md3.databinding.OpenCaseItemBreakdownLayoutBinding
...
import com.example.md3.view.home.view.HomeFragmentBreakDownDirections
import com.example.md3.view.home.view.HomeFragmentBreakDownDirections.Companion.actionHomeBreakDownFragmentToViewNewCasesFragment
...
class BreakdownCasesFragment : Fragment(R.layout.fragment_new_case_breakdown) {
    ...
    private var _binding: FragmentNewCaseBreakdownBinding? = null
    private val breakdownViewModel: BreakdownViewModel by inject()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentNewCaseBreakdownBinding.inflate(inflater, container, false)
        return binding.root
    ...
    private fun initAdapters() {
        newCasePagingAdapter = GenericPagingAdapter(
            R.layout.new_case_item_breakdown_layout,
            { view, item, position ->
                val binding = NewCaseItemBreakdownLayoutBinding.bind(view)
                ...
                binding.productNamevalue.text = item.serviceRequestDetails.productName
                binding.requestCreateDateValue.text = item.serviceRequestDetails.plannedDate
                ...
                binding.root.setOnClickListener {
                    val action =
                        actionHomeBreakDownFragmentToViewNewCasesFragment(
        ...
        upcomingVisitAdapter = GenericPagingAdapter(
            R.layout.open_case_item_breakdown_layout,
            { view, openCasesData, position ->
                val binding = OpenCaseItemBreakdownLayoutBinding.bind(view)
                binding.productNameValue.text = openCasesData.servicerequestdetails.productName
                binding.totalJourneyTimeValue.text = "-----"
                binding.customerNameTitle.text =
                    openCasesData.servicerequestdetails.customerDetails.name
                binding.customerNameValue.text = openCasesData.servicerequestdetails.caseId
                binding.totalVisitsValue.text =
                    openCasesData.servicerequestdetails.customerDetails.name
                ...
                binding.btnStop.setOnClickListener {
                    ...
                    breakdownViewModel.journeyVisitEndRoute(
                        caseId,
                ...
                binding.btnViewCase.setOnClickListener {
                    val action =
                        HomeFragmentBreakDownDirections.actionHomeBreakDownFragmentToBreakDownCurrentVisitFragment(
                            openCasesData.id,
        ...
        allCasesAdapter = GenericPagingAdapter(
            R.layout.all_cases_breakdown_layout,
            { view, closedCasesResult, position ->
                val binding = AllCasesBreakdownLayoutBinding.bind(view)
                ...
                if (closedCasesResult.status == "Closed") {
                    ...
                    val list = listOf<ItemModel>(
                        if (closedCasesResult.timeElapsed != null) {
                            ItemModel(
                                "Total Work Time",
                                "${
                                    closedCasesResult.timeElapsed.hours.toString().padStart(2, '0')
                                }:" +
                                        "${
                                            closedCasesResult.timeElapsed.minutes.toString()
                                                .padStart(2, '0')
                                        }"
                            )
                        } else {
                ...
                binding.groupCreateVisit.isVisible =
                    closedCasesResult.status == "Open" || closedCasesResult.status == "In-Progress"
                ...
                binding.root.setOnClickListener {
//                    val action = HomeFragmentDirections.actionHomeFragmentToViewOpenCasesFragment(
...
//                    findNavController().navigate(action)
                ...
                binding.btnViewCase.setOnClickListener {
                    val action =
                        HomeFragmentBreakDownDirections.actionHomeFragmentToViewOpenCasesBreakdownFragment(
                            closedCasesResult.id,
                            closedCasesResult.caseId
                        )
                    findNavController().navigate(action)
    ...
    private fun showRejectDialogBox(commissioningEngineerId: String) {
        ViewKotlinUtils.rejectAlertDialog(
            ...
            positiveButtonClickListener = {
                breakdownViewModel.changeCaseRequest(
...
//    fun handleStopJourney(openCasesData: VisitResult) {
//        Log.d("Tag", "handleStopJourney: ${openCasesData}")
//        openCasesData.latestJourneyDetails?.let {
//            Log.d("TAG", "handleStopJourney: " + it)
//            val list = (requireActivity() as MainActivity).getUpdatedLatAndLong()
//            breakdownViewModel.journeyVisitEndRoute(
//                openCasesData.id,
//                JourneyVisitRouteResponse(
...
//            )
//
//            breakdownViewModel.journeyVisitEndRouteLiveData.observe(viewLifecycleOwner) {
//                if(!it.isResponseHandled()){
//                    when (it.status) {
//                        Status.SUCCESS -> {
//                            Log.d("TAG", "EndRoute:JournetResponse " + it.data?.latestJourneyDetails?.id)
//                            Log.d("TAG", "EndRoute:CaseId " + it.data?.id)
//
//                            val caseId = it.data?.id ?: ""
//                            val journeyId = it.data?.latestJourneyDetails?.id ?:""
//
//                            openCasesData.latestJourneyDetails.let {
//                                breakdownViewModel.endJourney(
//                                    caseId,
//                                    Date().toFormattedTime(),
//                                    journeyId
//                                )
...
//    }
    ...
    private fun observe() {
        ...
        sharedViewModel.isRefreshRequiredNewCases.observe(viewLifecycleOwner){
            if(it == true){
                breakdownViewModel.refreshNewCases()
        ...
        sharedViewModel.isRefreshRequired.observe(viewLifecycleOwner)
        { it ->
            if (it == true) {
                breakdownViewModel.refreshUpcomingVisit()
                sharedViewModel.isRefreshRequired.postValue(false)
            }
        }

        breakdownViewModel.getCreateVisitLiveData.observe(viewLifecycleOwner)
        {
        ...
        when (tabType) {
            TabType.NEW -> {
                ...
                breakdownViewModel.getAllNewCases.observe(viewLifecycleOwner) {
            ...
            TabType.UPCOMING_VISITS -> {
                ...
                breakdownViewModel.getAllOpenCases.observe(viewLifecycleOwner) {
            ...
            TabType.ALL_CASES -> {
                ...
                breakdownViewModel.getAllCases.observe(viewLifecycleOwner) {
        ...
        breakdownViewModel.caseRequestLiveData.observe(viewLifecycleOwner)
        {
            if (!it.isResponseHandled()) {
                when (it.status) {
                    Status.SUCCESS -> {
                        ...
                        if (it.data?.status == ConstantStrings.accepted) {
                            Snackbar.make(
                                ...
                                "BreakDown Case Accepted",
                                Snackbar.LENGTH_LONG
                            )
                                .setAction("Open Case") {
                                    breakdownViewModel.refreshOpenCasesList()
        ...
        breakdownViewModel.startJourneyLiveData.observe(viewLifecycleOwner)
        {
            if (!it.isResponseHandled()) {
                when (it.status) {
                    Status.SUCCESS -> {
                        breakdownViewModel.journeyVisitRoute(
        ...
        breakdownViewModel.journeyVisitRouteLiveData.observe(viewLifecycleOwner)
        {
            if (!it.isResponseHandled()) {
                when (it.status) {
                    Status.SUCCESS -> {
        ...
        breakdownViewModel.endJourneyLiveData.observe(viewLifecycleOwner)
        {
            if (!it.hasBeenHandled) {
                when (it.status) {
                    Status.SUCCESS -> {
    ...
    companion object {
        ...
        fun newInstance(tabType: TabType): BreakdownCasesFragment {
            val fragment = BreakdownCasesFragment()
            val args = Bundle()
