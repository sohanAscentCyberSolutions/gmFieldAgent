...
import com.example.md3.databinding.FragmentPastVisitBreakdownBinding
import com.example.md3.databinding.PastVisitBreakdownLayoutBinding
...
class PastVisitBreakDownFragment : Fragment() {

    private var _binding: FragmentPastVisitBreakdownBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            tabType = it.getSerializable(BreakdownCasesFragment.ARG_TAB_TYPE) as TabType
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPastVisitBreakdownBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun initAdapter() {
        pastVisitAdapter = GenericPagingAdapter(
            R.layout.past_visit_breakdown_layout,
            { view, visitResult , position ->
                val binding = PastVisitBreakdownLayoutBinding.bind(view)
...
//                    val createVisitBottomSheet = CreateVisitBottomSheet(true,visitResult.servicerequestdetails) {
...
//                    bottomSheet.show(childFragmentManager, "")
                    ...
                    if (visitResult.status == "Completed" || visitResult.status == "Continued") {
                        ...
                        val action = ViewOpenCasesBreakDownFragmentDirections.actionViewOpenCasesBreakDownFragmentToBreakDownCurrentVisitFragment(visitResult.id, "Visit # " + visitResult.visitCount.toString())
            ...
            { view, visitResult, position ->
                ...
                binding.tvViewDetails.setOnClickListener {
                    ...
                    if (visitResult.status == "Completed" || visitResult.status == "Continued" ) {
                        val bundle = Bundle()
                        bundle.putString(VisitInputDetailsBottomSheet.VISIT_ID, visitResult.id)
                        bundle.putBoolean(VisitInputDetailsBottomSheet.IS_FROM_COMMISSION, false)
                        val bottomSheet = VisitInputDetailsBottomSheet()
                        bottomSheet.arguments = bundle
                        bottomSheet.show(childFragmentManager, "")
                    } else {
