...
class StopwatchService : Service() {
    companion object {
        ...
        const val START_FROM_TIME = "START_FROM_TIME"
        const val START_TIME_SECONDS = "START_TIME_SECONDS"
        const val STOP_TIME_ELAPSED = "STOP_TIME_ELAPSED"
    ...
    * */
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (action) {
            ...
            STOP -> {
                ...
                stopStopwatchAndShowTime(startTimeSeconds)
            }
            START_FROM_TIME -> {
                val startTimeSeconds = intent.getIntExtra(START_TIME_SECONDS, 0)
                startStopwatchFromTime(startTimeSeconds)
            }
        }
    ...
    private fun startStopwatchFromTime(startTimeSeconds: Int) {
        isStopWatchRunning = true
        timeElapsed = startTimeSeconds
        ...
        stopwatchTimer.scheduleAtFixedRate(object : TimerTask() {
            override fun run() {
                timeElapsed++
                val stopwatchIntent = Intent(STOPWATCH_TICK)
    ...
    * */
    ...
    private fun stopStopwatchAndShowTime(startTimeSeconds: Int) {
        isStopWatchRunning = false // Update the stopwatch status
        timeElapsed = startTimeSeconds // Set the final time

        // Cancel the stopwatch timer
        stopwatchTimer.cancel()

        // Send the final time to the activity
        sendStatus()

        // Update the notification with the final time
        updateNotification()
    }
